...
namespace Cenium.Reservations.Client.Windows.Actions
{
    ...
    /// </summary>
    [RegisterActionType("frontdesk.cancelaction")]
    public class CancelAction : AbstractActionHandler
    {
        ...
        protected override void OnExecute()
        {
            ...
            else if (rec["Status"].ToString() != "Confirmed" && rec["Status"].ToString() != "Created") 
            {
                MessageBox.Show("You can only cancel reservations in status Confirm or Created.");
                return;
            }
            ...
            {
                var executingtitlemessage = "Canceling. Please wait...";
                string executingMessageTitle = executingtitlemessage;
                string executingMessage = "Canceled All Shares...";
        ...
        private void CheckInImpl(Record request, string executingMessageTitle, string executingMessage)
        {
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show(string.Format("{0}\n{1}\n{2}", "Error occured when doing canceling.",
        ...
        private void Finished(ServiceOperationResult result)
        {
            ...
            else
            {
                EventDispatchManager.ExecuteOnUIThread(
                    (Action)delegate ()
                    {
                        MessageBox.Show(string.Format("{0}\n{1}\n{2}", "An error occured when doing canceling.",
